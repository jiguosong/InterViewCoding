...
vector<Point> GenerateRandomPoints(int count, int lowerH = 0, int upperH = 10, int lowerV = 0, int upperV = 10)
{
    srand(time(NULL));
...
TEST(geomtricmedian, normal1)
{
    geomtricmedian tc;

    vector<Point> points = GenerateRandomPoints(10);
    for (int i = 0; i < 10; i++) printf("[%d,%d] ", points[i].x, points[i].y);

    cout << endl;
    Point ans = tc.GeometricCenter(points);
