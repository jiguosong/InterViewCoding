...
#include "../src/buildtreefromsortedarray.h"
...
TEST(buildtreefromsortedarray, normal1)
{
	buildtreefromsortedarray tc;

	Array<int> x(10, 1, 10);
	vector<int> array = x.getRandomArray();
	std::sort(array.begin(), array.end(), [](int a, int b){return a <= b;});
	PrintVector(array);
	TreeNode *root = tc.sortedArrayToBST(array);
