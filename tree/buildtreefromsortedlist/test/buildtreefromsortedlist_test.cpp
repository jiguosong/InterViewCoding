...
#include "../src/buildtreefromsortedlist.h"
...
TEST(buildtreefromsortedlist, normal1)
{
	buildtreefromsortedlist tc;

	List<int> x(10, 1, 100);
	ListNode<int> *head = x.head;
	head->print_list();
	head = SortList(head);
	head->print_list();

	TreeNode *root = tc.sortedListToBST(head);
