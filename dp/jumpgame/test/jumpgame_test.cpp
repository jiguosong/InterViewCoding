...
TEST(jumpgame, normal1)
{
	jumpgame tc;

	vector<int> steps = {2,3,1,1,4};

	ASSERT_TRUE(tc.JumpToLast(steps));
}

TEST(jumpgame, normal2)
{
	jumpgame tc;

	vector<int> steps = {3,2,1,0,4};

	ASSERT_FALSE(tc.JumpToLast(steps));
}
