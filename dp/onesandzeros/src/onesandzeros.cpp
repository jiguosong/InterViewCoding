...
want to pursue. For now, suppose you are a dominator of m 0s and n 1s respectively. On the other hand,
    ...
    for (auto &s : strs) {
        ...
        for (int i = m; i >= zeros; --i) {
            for (int j = n; j >= ones; --j) {
                dp[i][j] = std::max(dp[i - zeros][j - ones] + 1, dp[i][i]);
            }
        }
