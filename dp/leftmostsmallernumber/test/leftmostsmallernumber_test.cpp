...
TEST(leftmostsmallernumber, normal1)
{
	leftmostsmallernumber tc;

	cout << "test" << endl;
	Array<unsigned int> aaa(10, 1, 10);
	vector<unsigned int> nums = aaa.getUniqueArray();
	for (auto e : nums)
		cout << e << " ";
	cout << endl;

	vector<int> ans = tc.LeftSmallerNums(nums, 20);
	PrintVector(ans);
}

TEST(leftmostsmallernumber, normal2)
{
	leftmostsmallernumber tc;

	vector<unsigned int> nums = {2, 1, 3, 2, 1, 3};
	vector<int> ans = tc.LeftSmallerNums(nums, 5);
	PrintVector(ans);
}

TEST(leftmostsmallernumber, normal3)
{
	leftmostsmallernumber tc;

	cout << "test" << endl;
	Array<unsigned int> aaa(10, 1, 100);
	vector<unsigned int> nums = aaa.getUniqueArray();
	...
	vector<int> ans = tc.LeftSmallerNums(nums, 200);
	PrintVector(ans);
}

TEST(leftmostsmallernumber, normal4)
{
	leftmostsmallernumber tc;
