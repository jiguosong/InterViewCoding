...
public:
    ...
    {
        if (entry == NULL) entry = new DeletedEntry();
        return entry;
    }
};
...
public:
    ...
    int get(int key)
    {
        ...
        if (table[hash] == NULL || hash == initialHash) return -1;
    ...
    {
        for (int i = 0; i < TABLE_SIZE; i++) {
            if (table[i] != NULL && table[i] != DeletedEntry::getUniqueDeletedEntry()) delete table[i];
        }
        delete[] table;
    }
